...
export class ParseBulkQuestions {
  public async parseExcel(fileBuffer: Buffer) {
    try {
      ...
      // 2. Convert to JSON array (rows)
      const data: any[][] = XLSX.utils.sheet_to_json(sheet, { header: 1 });

      const parsedQuestions = [];

      // 3. Loop through rows (start at 1 to skip headers)
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        // Expected format: Col A=Question, Col B-E=Options
        if (!row[0]) continue;

        const questionText = String(row[0]).trim();

        // Collect options from remaining columns
        const optionsRaw = row.slice(1);
        const options = optionsRaw
            .filter((opt: any) => opt) // Remove empty cells
            .map((opt: string) => ({
              id: uuidv4(),
