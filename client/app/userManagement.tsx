...
import { useSafeAreaInsets } from "react-native-safe-area-context"; // ✅ Added for safe area fix
...
export default function UserManagement() {
  ...
  const axiosInstance = useAuthStore((state) => state.axiosInstance);
  const { addUser, uploadBulkUsers } = useAuthStore();
  const insets = useSafeAreaInsets(); // ✅ dynamic top inset
  ...
  return (
      <SafeAreaView style={styles.container}>
        {/* ✅ Safe area + StatusBar fix */}
        <StatusBar translucent backgroundColor="transparent" barStyle="light-content" />

        {/* ✅ Header extended behind Dynamic Island */}
        <View style={[styles.headerArc, { paddingTop: insets.top + 16 }]}>
          <View style={tw`absolute top-4 left-4 z-10 flex-row items-center gap-4`}>
        ...
        >
          ...
          {loading ? (
              ...
              (filteredUsers.length ? filteredUsers : users).map((user, index) => (
                  <View
                      key={user.id || user._id || index} // ✅ unique key fix
