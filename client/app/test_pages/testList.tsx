...
// ✅ CHANGE: Use axiosInstance from useAuthStore instead of separate import
import { useAuthStore } from '../../store/useAuthStore';
...
const TestList = () => {
  ...
  // ✅ CHANGE: Use the same axiosInstance as login/signup (from useAuthStore)
  const axiosInstance = useAuthStore((state) => state.axiosInstance);

  useEffect(() => {
    const fetchTests = async () => {
      try {
        console.log('Fetching assessments from:', axiosInstance.defaults.baseURL + '/assessments'); // Debugging
        const res = await axiosInstance.get('/assessments'); // ✅ Works with auth/session cookies
        setTests(res.data.reverse());
